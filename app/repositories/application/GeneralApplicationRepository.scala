  ...
  private def findProgress(document: BSONDocument, applicationId: String): ProgressResponse = {
    ...
    (document.getAs[BSONDocument]("progress-status") map { root =>
      ...
      ProgressResponse(
        ...
        phase1TestsInvited = getProgress(ProgressStatuses.Phase1Tests.INVITED.toString),
        phase1TestsStarted = getProgress(ProgressStatuses.Phase1Tests.STARTED.toString),
        phase1TestsCompleted = getProgress(ProgressStatuses.Phase1Tests.COMPLETED.toString),
        phase1TestsExpired = getProgress(ProgressStatuses.Phase1Tests.EXPIRED.toString),
        phase1TestsResultsReceived = getProgress(ProgressStatuses.Phase1Tests.RESULTS_RECEIVED.toString),
