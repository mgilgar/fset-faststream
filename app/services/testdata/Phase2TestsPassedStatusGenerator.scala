...
import model.ProgressStatuses.PHASE2_TESTS_PASSED
...
object Phase2TestsPassedStatusGenerator extends Phase2TestsPassedStatusGenerator {
  override val previousStatusGenerator = Phase2TestsCompletedStatusGenerator
...
trait Phase2TestsPassedStatusGenerator extends ConstructiveGenerator {
  ...
  def generate(generationId: Int, generatorConfig: GeneratorConfig)(implicit hc: HeaderCarrier, rh: RequestHeader) = {
    for {
      ...
      _ <- appRepository.addProgressStatusAndUpdateAppStatus(candidate.applicationId.get, PHASE2_TESTS_PASSED)
    } yield candidate
  }
}
