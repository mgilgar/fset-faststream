...
import model.events.{ AuditEvents, DataStoreEvents, EventTypes }
...
trait Phase1TestService extends OnlineTestService with Phase1TestConcern with ResetPhase1Test {
  ...
  override def retrieveTestResult(testProfile: RichTestGroup)(implicit hc: HeaderCarrier, rh: RequestHeader): Future[Unit] = eventSink {

    def insertTests(testResults: List[(TestResult, CubiksTest)]) = {
      ...
      }).map(_ => DataStoreEvents.UserRegisteredForOnlineExercise(application.applicationId))
    }

    def maybeUpdateProgressStatus(appId: String)(implicit hc: HeaderCarrier, rh: RequestHeader) = {
      testRepository.getTestGroup(appId).flatMap { eventualProfile =>
        ...
        if (latestProfile.activeTests.forall(_.testResult.isDefined)) {
          testRepository.updateProgressStatus(appId, ProgressStatuses.PHASE1_TESTS_RESULTS_RECEIVED).map { _ =>
            AuditEvents.ProgressStatusModified("appId" -> appId, "newStatus" -> ProgressStatuses.PHASE1_TESTS_RESULTS_RECEIVED) :: Nil
          }
        } else {
    ...
    for {
      ...
      maybeAuditEvents <- maybeUpdateProgressStatus(testProfile.applicationId)
    } yield {
      AuditEvents.OnlineExerciseResultsRetrievedForSchedule("applicationId" -> testProfile.applicationId) ::
      DataStoreEvents.OnlineExerciseResultsRetrievedForSchedule(testProfile.applicationId) :: maybeAuditEvents
    }
  }
